<template>
    <div :key="refreshVPNUnit">
        <div id="wrapper-password-chge" :key="langKey" :class="{ hidden : !showVPNUnit }">
            <div class="cont-password cont-password-chge">
                <div class="cont-title-password">
                    ...
                    <fa :icon="['fas', 'times']" @click="closeVPNUnit" />
                </div>
                <div class="cont-xl">
...
export default {
    ...
    computed: {
        showVPNUnit(){
            return this.$store.state.showVPNUnitStore;
        },

        // recharge composant à chaque reouverture
        refreshVPNUnit(){
            this.unitSelectedKey = this.$store.state.showVPNUnitStore;
        },
    ...
    watch: {
        ...
        //  chaque fois que langStore change, la requête pour la langue des textes s'exécute.
        unitSelectedKey(newUnitSelectedKey, oldUnitSelectedKey) {
    ...
    methods: {
        ...
        getVPNs(){
            if (location.hostname === "localhost" || location.hostname === "127.0.0.1"){
                ...
                axios
                .get(`http://localhost:3000/api/entities/vpn/${this.unit}`)
                .then((response) => {
                ...
                const axios = require('axios').default;
                axios
                .get(`/api/entities/${this.unit}/vpn`)
                .then((response) => {
                    this.vpns = response.data;
                })
                .catch((error) => console.log(error));
        ...
        closeVPNUnit(){
            ...
            this.newDescri = "";
            this.vpns = [];
            this.$store.commit('SETSHOWVPNUNITSTORE', false)
        },
        ...
        getTexts() {
            ...
            .get("./I18n.json")
            .then((response) => {
                this.texts = response.data[this.$store.state.lang].savevpnunit;
            })
            .catch((error) => console.log(error));
        },
        ...
        saveVPNUnit(){
            if (location.hostname === "localhost" || location.hostname === "127.0.0.1"){
                ...
                const axios = require('axios').default;
                axios
                .patch(`http://localhost:3000/api/entities/vpn/${this.unit}` , {vpns: this.vpns})
                .then((response) => {
                ...
                const axios = require('axios').default;
                axios
                .patch(`/api/entities/${this.unit}/vpn`, {vpns: this.vpns})
                .then((response) => {
